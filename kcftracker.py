...
class KCFTracker:
    ...
    def getFeatures(self, image, inithann, scale_adjust=1.0):
        ...
        if(inithann):
            ...
            if(self._hogfeatures):
                self._tmpl_sz[0] = \
                    int(self._tmpl_sz[0]) // (2 * self.cell_size) * \
                    2 * self.cell_size + 2 * self.cell_size
                self._tmpl_sz[1] = \
                    int(self._tmpl_sz[1]) // (2 * self.cell_size) * \
                    2 * self.cell_size + 2 * self.cell_size
            else:
    ...
    def detect(self, z, x):
        ...
        if(pi[0] > 0 and pi[0] < res.shape[1] - 1):
            p[0] += self.subPixelPeak(res[pi[1], pi[0] - 1],
                                      pv, res[pi[1], pi[0] + 1])
        if(pi[1] > 0 and pi[1] < res.shape[0] - 1):
            p[1] += self.subPixelPeak(res[pi[1] - 1, pi[0]],
                                      pv, res[pi[1] + 1, pi[0]])
    ...
    def init(self, roi, image):
        ...
        self._prob = self.createGaussianPeak(
            self.size_patch[0], self.size_patch[1])
    ...
    def update(self, image):
        ...
        loc, peak_value = self.detect(
            self._tmpl, self.getFeatures(image, 0, 1.0))
